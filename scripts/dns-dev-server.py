...
try:
    from dnslib import CNAME, MX, NS, QTYPE, RR, SOA, TXT, A, DNSHeader, DNSRecord
except ImportError:
    print(
        "Error: dnslib not installed. Install with: pip install dnslib", file=sys.stderr
    )
    sys.exit(1)
...
class DynamicDNSServer:
    ...
    def __init__(
        self, host="127.0.0.1", port=5353, api_port=8000, upstream_dns="8.8.8.8"
    ):
        self.host = host
        self.port = port
        ...
        async def add_zone(zone: DNSZoneModel):
            ...
            try:
                ...
                for i, rec in enumerate(zone.records):
                    ...
                    record_name = (
                        rec.name if rec.name and rec.name != "@" else ""
                    )  # Empty string for zone apex
                    ...
                    self.zones[zone.domain][record_name][record_type].append(
                        {"value": rec.value, "ttl": rec.ttl, "priority": rec.priority}
                    )

                print(f"Added zone: {zone.domain} with {len(zone.records)} records")
                return {
                    "message": f"Zone {zone.domain} added successfully",
                    "domain": zone.domain,
                }
    ...
    def _handle_zone_record(self, request, qname, addr, qtype_int):
        ...
        for record in records:
            ...
            if qtype_name == "A":
                reply.add_answer(
                    RR(request.q.qname, rtype=QTYPE.A, ttl=ttl, rdata=A(value))
                )
            elif qtype_name == "TXT":
                # Handle TXT records properly - need bytes
                txt_value = value.encode() if isinstance(value, str) else value
                reply.add_answer(
                    RR(request.q.qname, rtype=QTYPE.TXT, ttl=ttl, rdata=TXT(txt_value))
                )
                ...
                reply.add_answer(
                    RR(
                        ...
                        rtype=QTYPE.MX,
                        ttl=ttl,
                        rdata=MX(label=value, preference=priority),
                    )
                )
            elif qtype_name == "NS":
                reply.add_answer(
                    RR(request.q.qname, rtype=QTYPE.NS, ttl=ttl, rdata=NS(label=value))
                )
            elif qtype_name == "SOA":
                reply.add_answer(
                    RR(
    ...
    def _handle_mx_record(self, request, qname, addr):
        ...
        self._send_dns_response(
            request, addr, QTYPE.MX, MX(label="maildev", preference=10)
        )
    ...
    def handle_query(self, data, addr):
        ...
        try:
            ...
            if self.should_pass_through(qname):
                print(
                    f"DNS query: {qname} ({qtype}) from {addr} -> UPSTREAM (whitelist)"
                )
                response_data = self.forward_to_upstream(data)
                if response_data:
                    self.socket.sendto(response_data, addr)
    ...
    def start_dns_server(self):
        ...
        try:
            ...
            print(
                f"Error: Cannot bind to port {self.port}. Try with sudo or use a different port.",
                file=sys.stderr,
            )
            sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=5353, help="DNS port (default: 5353)"
    )
    parser.add_argument(
        "--api-port", type=int, default=8000, help="API port (default: 8000)"
    )
    parser.add_argument(
        "--upstream", default="8.8.8.8", help="Upstream DNS server (default: 8.8.8.8)"
    )
